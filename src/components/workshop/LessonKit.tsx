...
export function LessonMeta({
  ...
  return (
    <div className="rounded-xl border border-black/10 bg-white p-4 dark:border-[#2a2a3a] dark:bg-[#12121a]">
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-wider">
        ...
        {duration && (
          <span className="rounded-full bg-zinc-100 px-2.5 py-1 text-zinc-500 dark:bg-[#1a1a25] dark:text-zinc-400">
            {duration}
          </span>
        )}
      </div>
...
export function ConceptCard({
  ...
  title: ReactNode;
  children: ReactNode;
  tone?: "neutral" | "red" | "green" | "amber" | "blue" | "purple";
}) {
  const tones = {
    neutral:
      "border-black/10 bg-white dark:border-[#2a2a3a] dark:bg-[#12121a]",
